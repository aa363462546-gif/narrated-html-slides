...
通用 reveal 只负责内容块入场：元素从下方 `26px` 淡入，使用 `.75s` 时长和 `var(--ease)` 缓动；`d1`、`d2`、`d3`、`d4` 依次对应 `0.08s`、`0.18s`、`0.30s`、`0.44s`。它适合 eyebrow、标题、正文、卡片等完整内容块，不用来制造持续运动或改变版式尺寸。

线条动效只表达阅读关系已经建立：roadmap 和 process-strip 先展开主线，阶段卡片使用 `.44s`、`.56s`、`.68s`、`.80s` 的延迟依次进入；system-map 再按“核心 → 连接线 → 分支”出现。线条从起点向外一次性展开，不循环、不闪烁，也不承担文字入场；compare-table 使用整体 reveal，保持表格内部结构稳定。

柱形动效只表达数值从基线形成：bar-chart 的柱形以底部为锚点生长，不改变图表网格、标签或其他内容的位置。数值递增只服务于已有的主指标，必须保留原始数字格式和单位，例如 `%`、`×`、`M`；不能把单位拆掉、改写或变成另一段文字。metric-result 的核心数值、metric-grid 的卡片与数值按阅读顺序播放，进入和重访页面都从起始状态重播。

所有动效都挂在现有 `SlidePresentation.go()` 的页面激活流程上，页面离开时复位，回到页面时重新播放。设置 `prefers-reduced-motion: reduce` 时，reveal、线条、柱形和数字都直接显示最终状态。不要新增弹跳、旋转入场、霓虹闪烁、持续漂浮或与 Field Notes A 无关的转场；植物装饰也应安静地服务构图，不成为持续抢注意力的动画主体。
